...
nhoods <- st_read("https://raw.githubusercontent.com/mafichman/musa_5080_2023/main/Week_4/neighborhoods/bost_nhoods.geojson") %>%
...
```{r price_map, fig.height=3, fig.width=3}
# ggplot, reorder
...
bostonCrimes.sf <-
  bostonCrimes %>%
    filter(OFFENSE_CODE_GROUP == "Aggravated Assault",
           Lat > -1) %>%       # this is like putting NA, placeholder for a value that is unknown
...
boston.sf$crimes.Buffer <- boston.sf %>% 
    st_buffer(660) %>% 
    aggregate(mutate(bostonCrimes.sf, counter = 1),., sum) %>%  # chotto wakannnai, there's a section on this in the book
    pull(counter)
...
# comp : feature that describes predicted value of a home based on prices of nearest houses that were sold
# might be important to compare after filter for other features of the house (like number of bedrooms)
# square footage is a great proxy
# how to choose the best k? it depends lol mostly depends on magnitude of the target, 
```
...
ggplot() + geom_sf(data = nhoods, fill = "grey40") +
  ...
  scale_alpha(range = c(0.00, 0.35), guide = "none") + # feathering as intensity gets lower
...
Run these code blocks... Notice the use of `st_drop_geometry()`, this is the correct way to go from a `sf` spatial dataframe to a regular dataframe with no spatial component.
...
Pearson's r Learning links: \* [Pearson Correlation Coefficient (r) \| Guide & Examples](https://www.scribbr.com/statistics/pearson-correlation-coefficient/) \* [Correlation Test Between Two Variables in R](http://www.sthda.com/english/wiki/correlation-test-between-two-variables-in-r)
