...
p3 <- fviz_cluster(k4, geom = "point",  data = data_scaled) + ggtitle("k = 4") # my graph looks flipped (across y = 0 intercept) as compared to professor's
...
ggplot(cent5, aes(Black18 ,White18, color = cluster, label = cluster)) +
  geom_point() +
  geom_label() +
  theme_bw()  # visualizing clusters' characteristics (2 variables at a time)
...
caret::confusionMatrix(testProbs$predOutcome, testProbs$cluster1) # metric to look at balance between sensitivity/specificity
```
