...
- regression for count where a transformation from normal distribution to a skewed one might be needed
...
fishnet <- 
  st_make_grid(chicagoBoundary,
               cellsize = 500,  # use st_crs(chicagoBoundary) to see that this is 500 m
               square = TRUE) %>% # hex look cooler & distance from border to center (raidius ish) is more consistent in hex
  .[chicagoBoundary] %>%            # fast way to select intersecting polygons
  st_sf() %>%              # turn object into sf object, in this case it was not necessary because 
  mutate(uniqueID = 1:n())  # neeed to assign grid cells uniqueIDs to tie features and predictions back to each other
...
crime_net <- 
  dplyr::select(burglaries) %>%    # only takes the geometry of the object
  mutate(countBurglaries = 1) %>%  # assign 1 to each instance of burglary
  aggregate(., fishnet, sum) %>%   # summing the amount of burglary instances that happens in each grid
  mutate(countBurglaries = replace_na(countBurglaries, 0),
         uniqueID = 1:n(),
         cvID = sample(round(nrow(fishnet) / 24),    # divide by 24 to because there are 2458 rows, 100 per CV fold 
                       size=nrow(fishnet), replace = TRUE))   # assign random CV ID
...
# check that our code is working by manually checking that the amount of burglaries in a grid matches the color assigned by countBurglaries
...
```{r}
# vars_net: network of variables, wide format of data, counting up various variables for each grid
...
vars_net <- vars_net %>%
    mutate(Abandoned_Cars.nn = nn_function(st_c(st_coid(vars_net)), # always use centroid of polygons before extracting coords
                                           st_c(abandonCars),
                                           k = 3))    # average distance to the nearest 3 abandoned cars to centroid of grids
# using plot(vars_net) throughout to see what the variables look like across space
# in this case, Abandoned_Cars.nn looks like an inverse map of Abandoned_Cars (because we're measuring distance for .nn)
```
...
vars_net.long.nn <- 
  dplyr::select(vars_net, ends_with(".nn")) %>%
    gather(Variable, value, -geometry)    # transform to long data
...
final_net <-
  st_centroid(final_net) %>%  # joining polygons to polygons is a pain, so use centroids of fishnet grids or point
    st_join(dplyr::select(neighborhoods, name)) %>%  # you don't need by = "uniqueID", without it we get a warning msg abt geom
...
final_net.nb <- poly2nb(as_Spatial(final_net), queen=TRUE) # poly2nb() requires sp object
...
# print(final_net.weights, zero.policy=TRUE) # have to add zero.policy=T for any function calling final_net.weights
```

```{r}
## see ?localmoran
local_morans <- localmoran(final_net$Abandoned_Cars, final_net.weights, zero.policy=TRUE) %>% # no "join by" here so you have to make sure that you're inputting things correctly (vectors/matrices have the same order)
...
error_by_reg_and_fold <- 
  reg.ss.spatialCV %>%
    ...
    summarize(Mean_Error = mean(Prediction - countBurglaries, na.rm = T),
              ...
              SD_MAE = mean(abs(Mean_Error), na.rm = T)) %>% # why isn't sd working? get all NAs for SD_MAE
  ungroup()
