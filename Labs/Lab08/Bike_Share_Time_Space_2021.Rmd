...
study.panel <- 
  expand.grid(interval60=unique(dat_census$interval60), # this populates the combinations of uniq hours & uniq stations
...
ride.panel <- 
  ride.panel %>% 
  arrange(from_station_id, interval60) %>%  # this arrangement is important
  mutate(lagHour = dplyr::lag(Trip_Count,1), # lag of tripcount 1 row back (the hour before), this is why arrangement is imp
