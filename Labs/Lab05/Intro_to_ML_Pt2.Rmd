...
inTrain <- createDataPartition(
              ...
              # defining y gives the function an idea of how to stratify predictors somewhat evenly, use this carefully
              p = .60, list = FALSE) # p = .60 = split to 60/40 ratio for train/test
boston.training <- boston.sf[inTrain,] # defining training set
boston.test <- boston.sf[-inTrain,]    # defining testing set
...
# if you get an error (about testing subset having features that didn't exist in training set), you can assign a big list of factors/levels
# collapsing features/categorical variables is a good approach to solve this as well
...
neighborList <- knn2nb(knearneigh(coords, 5)) # k nearest neighbor and then convert to neighbors list
# ^^ inner function gives the actual coordinates of k nearest neighbors to each point of interest
spatialWeights <- nb2listw(neighborList, style="W") # convert to list of weights, calculates weights based on proximity

boston.sf$lagPrice <- lag.listw(spatialWeights, boston.sf$SalePrice) # can lag other variables as well
...
```{r}
# doing the same thing as above but just for the test set and lag errors
...
# moran.mc() does the permutation for us
...
```{r}
# better representation of the same thing we were seeing in the plot before
st_drop_geometry(bothRegressions) %>%
