...
####  Figure 2. Map of City Council Districts
```{r fig2 city council districts}
...
#### Figure 3. Permit Count on Fishnet by Year
```{r fig3 change in count_permits, fig.height=6, fig.width=10}
...
- Total Population (total_pop): as population increases in a census tract, so too will housing pressures which can in turn lead to new construction of housing.
- Median Rent (med_rent): a clear marker of development pressure, rent prices will increase as a neighborhood becomes more expensive. It is likely that the cost of amenities in the area, including groceries will increase. It can also be used to ask 'does higher rent in an area induce more development' as developers seek to capitalize on higher prices?
- Education (bachelors25): Number of 25+ year olds with a Bachelor's degree
...
- Children (children): number of households with children under 18 years present. This can be a marker of how family friendly a neighborhood is.
- Vehicles available (vehicles_avail): self-explanatory. This feature tells us about what kinds of neighborhoods allow its residents to have the luxury of owning a personal vehicle, rather than having to use public transportation.
...
#### Figure 4. Correlation Matrix of all Predictors
```{r fig4 correlation matrix, fig.height=6, fig.width=10}
...
#### Figure 5. Permit Counts Distribution
```{r fig4 poisson}
...
#### Figure 6. Map of Predictors
```{r fig6 small mult map, fig.height=18, fig.width=10}
...
#### Figure 7. Adding indicators of significant spatial processes
```{r fig 7 small mult map local morans i, fig.height=10}
...
#### Figure 8. Scatterplots of Predictors
```{r fig8 small mult scatter, fig.height=21, fig.width=10}
correlation.long <-
...
- Count of permits from 1 year before (count_permits18)
- Count of permits from 2 years before (count_permits17)
- Count of permits from 3 years before (count_permits16)
- Count of permits from 4 yeasr before (count_permits15)
- Count of incident reports related to building construction (BuildingConstruction)
- Count of incident reports related to illegal dumping (IllegalDumping)
- Count of incident reports related to dangerous sidewalks (DangerousSidewalk)
- Count of incident reports related to rubbish/recyclable material collection (RubbishRecyclableMaterialCollection)
- Vehicles Available (vehicles_avail)
- Median Rent (med_rent)
- Renter-Occupied housing (renter_occ)
- Count of incident reports related to vacant house or commercial building (VacantHouseorCommercial)
- Total population (total_pop)
- Number of people who live in the same house as 1 yr ago (samehouse1yr_metro)
- Count of incident reports related to street trees (StreetTrees)
- Number of households with children under 18 present (children)
- Distance to the three nearest locations where an incident regarding parks and recreation maintenance was reported (parksNrec.dist)
- Number of 25+ year old people with Bachelor's degree (bachelors25)
...
#### Figure 9. Map of Model Errors
```{r fig9 small mult map errors, fig.height=8, fig.width=10}
reg.summary <- rbind(
  mutate(reg.CV,
...
## Real vs Predicted 2019 Permit Count
...
#### Figure 10. Real vs Predicted 2019 Permit Count
```{r fig 10 predicted vs real}
reg.summary %>% 
...
## Generalization across different Neighborhood Contexts

#### Figure 12. Map of Neighborhoods (Race Context, 2019)
```{r fig12 raceContext map}
...
#### Figure 13. Map of Neighborhoods (Income Context, 2019)
```{r fig13 incomeContext map}
