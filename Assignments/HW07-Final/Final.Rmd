...
=======
- Rent (as %age of Income): a markeer of affordabiliyt, it is possible that by building in rent-burdened communities 
...
## Figure 7. Correlation Matrix of all possible risk factors
...
>>>>>>> 5097e5bb2c0f527552d6019ea3c0d7f4ba308bd0
...
Figure 9 shows the real vs predicted counts of new construction permits in 2019 (see figure 9). The model here tends to over predict the number of construction permits that will occur in the Center City area, yet underpredicts in some north-west parts of the city. The real permit count (in comparison to the predicted permit count) is more distributed, indicating that it is possible the predicted permit count is being thrown off by extreme values. 
...
racebyNet <- reg.summary %>% 
  ...
  st_drop_geometry() %>%
  group_by(Regression, RaceContext)%>%
  left_join(racebyNet,RaceContext, by='GEOID')
  #summarize(mean.Error = mean(Error, na.rm = T)) 
  #spread(RaceContext, mean.Error)
...
Similarly, we tested the generalizability of our model in an income context.The results of this context analysis indicate that where 
