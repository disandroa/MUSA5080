...
ggplot()+
  geom_sf(data=all_net,aes(fill=all_net$ all_net$`Total Population 14`))+
...
With all of the variables loaded into 
...
<<<<<<< Updated upstream
Since the outcome we are interested in is a count of permits issued in different regions of Philadelphia, we use a Poisson regression model. We created two types of models -- one with just risk factors as the predictors and another with both risk factors and spatial process.
=======
...
```{r fig5 small mult map, fig.height=18}
...
```{r fig7 small mult scatter, fig.height=21}
correlation.long <-
