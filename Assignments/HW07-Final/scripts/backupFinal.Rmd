...
tracts13 <- 
  ...
  st_transform(crs = 2272) %>%
  dplyr::select(!matches("M$")) %>% 
  rename(total_pop_13 = B01001_001E,
         med_hh_inc_13 = B19013_001E,
         white_pop_13 = B02001_002E,
         med_rent_13 = B25058_001E,
         bachelors25_13 = B15003_022E,
         pct_rent_hhinc_13 = B25071_001E
  ) %>%
  mutate(pct_white_13 = ifelse(total_pop_13 > 0, white_pop_13 / total_pop_13,0)) %>% 
  dplyr::select(!matches("^B|white_pop|GEOID|NAME",ignore.case = F)) %>% 
  st_centroid() %>% 
  gather("Legend","value",-geometry)
...
tracts15 <- 
  ...
  st_transform(crs = 2272) %>%
  dplyr::select(!matches("M$")) %>% 
  rename(total_pop_15 = B01001_001E,
         med_hh_inc_15 = B19013_001E,
         white_pop_15 = B02001_002E,
         med_rent_15 = B25058_001E,
         bachelors25_15 = B15003_022E,
         pct_rent_hhinc_15 = B25071_001E
  ) %>%
  mutate(pct_white_15 = ifelse(total_pop_15 > 0, white_pop_15 / total_pop_15,0)) %>% 
  dplyr::select(!matches("^B|white_pop|GEOID|NAME",ignore.case = F)) %>% 
  st_centroid() %>% 
  gather("Legend","value",-geometry)
...
tracts17 <- 
  ...
  st_transform(crs = 2272) %>%
  dplyr::select(!matches("M$")) %>% 
  rename(total_pop_17 = B01001_001E,
         med_hh_inc_17 = B19013_001E,
         white_pop_17 = B02001_002E,
         med_rent_17 = B25058_001E,
         bachelors25_17 = B15003_022E,
         pct_rent_hhinc_17 = B25071_001E
  ) %>%
  mutate(pct_white_17 = ifelse(total_pop_17 > 0, white_pop_17 / total_pop_17,0)) %>% 
  dplyr::select(!matches("^B|white_pop|GEOID|NAME",ignore.case = F)) %>% 
  st_centroid() %>% 
  gather("Legend","value",-geometry)
...
tracts19_2 <- 
  ...
  st_transform(crs = 2272) %>%
  dplyr::select(!matches("M$")) %>% 
  rename(total_pop_19 = B01001_001E,
         med_hh_inc_19 = B19013_001E,
         white_pop_19 = B02001_002E,
         med_rent_19 = B25058_001E,
         bachelors25_19 = B15003_022E,
         pct_rent_hhinc_19 = B25071_001E
  ) %>%
  mutate(pct_white_19 = ifelse(total_pop_19 > 0, white_pop_19 / total_pop_19,0)) %>% 
  dplyr::select(!matches("^B|white_pop|GEOID|NAME",ignore.case = F)) %>% 
  st_centroid() %>% 
  gather("Legend","value",-geometry)

demos_net <- rbind(tracts13,tracts15,tracts17,tracts19_2) %>% 
  ...
  dplyr::select(-`<NA>`) %>%
  ungroup() %>% 
  cbind(permit_net13 %>% dplyr::select(count_permits13) %>% st_drop_geometry(),
        permit_net15 %>% dplyr::select(count_permits15) %>% st_drop_geometry(),
        permit_net17 %>% dplyr::select(count_permits17) %>% st_drop_geometry(),
        permit_net19 %>% dplyr::select(count_permits19) %>% st_drop_geometry())

vars_net.long <- gather(demos_net %>% dplyr::select(matches("geom|uniq|med_rent|count|bache")), 
                        variable, value, -geometry, -uniqueID)

# vars <- unique(vars_net.long$variable)
vars <- c("count_permits13","med_rent_13","bachelors25_13",
          "count_permits15","med_rent_15","bachelors25_15",
          "count_permits17","med_rent_17","bachelors25_17",
          "count_permits19","med_rent_19","bachelors25_19")
varList <- list()
...
do.call(grid.arrange,c(varList, ncol = 3, top = "Permit Count, Percent of White Population, Rent as Percentage of Income", bottom = "Figure 5."))
...
ggplot()+
  ...
  labs(title = "Total Population in 2014", fill = "Total Population in 2014")
# ggplot()+
